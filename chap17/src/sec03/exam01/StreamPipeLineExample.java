...
public class StreamPipeLineExample {
	...
	public static void main(String[] args) {
		// 불변 리스트를 만드는 2가지 방법
		// 1. Arrays.asList(크기 불변 리스트): 요소 변경 가능(set)/크기 변경 불가 + null 허용
...
//		);
		...
		// Quiz: 학생 객체를 학생의 이름으로 변환해서(중간 처리) 출력해보기(최종 처리)
		// 학생 객체를 String으로 매핑하면 됨
		// 힌트: map(), forEach()
		list.stream()
			.map(student -> student.getName()) // 중간 처리
			.forEach(name -> System.out.println(name)); // 최종 처리
